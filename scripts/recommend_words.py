...
def recommend(data, freq_by_simp, top=3, show_all=False):
    """Return a sorted list of (rank, simp, trad, count, pages) for characters
    in the book that are not in the priority list and have a frequency rank."""
...
def recommend_words(
    ...
    results, total, not_learnt, in_top = recommend(
        data, freq_by_simp, top=top, show_all=all
    )
